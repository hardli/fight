			...
			else 
			{
				...
				else
				{
					...
					if (pVehicle->u16FrameCnt>MAX_CUT)
					{
						...
						{
							// 20170908 修改逻辑漏洞
							if(pVehicle->FrShape.LoDaFeiR>50)
							{
								pVehicle->JudgeSet.u8JudgePattern=FIRST_KECHE;
							...
							else
							{
								...
								else
								{
									//货2
									pVehicle->JudgeSet.u8JudgePattern=SECOND_HUOCHE;
									pVehicle->JudgeSet.u8JudgeRule=125;
								}
							}
						}
					}
					else
					{
						...
						if (pVehicle->EdgeSet.u16HeadHeight <= 2200)  //车高2200以下	
						{
							//货1
							pVehicle->JudgeSet.u8JudgePattern=FIRST_HUOCHE;
							pVehicle->JudgeSet.u8JudgeRule=126;
						}
						else
						{
							//货2
							pVehicle->JudgeSet.u8JudgePattern=SECOND_HUOCHE;
							pVehicle->JudgeSet.u8JudgeRule=127;
						}
					}
...
void ThreeAxleClassify(VehicleSet *pVehicle)
{
	if (pVehicle->AxleSet.AxleSum == THREE_AXLE)
	{
		...
		{
			//货3
			if ((pVehicle->FeatureSet.IsHtRough
				||pVehicle->FeatureSet.IsSideUiform>0)
				&&pVehicle->FeatureSet.IsGlassWin
				&&pVehicle->FeatureSet.IsCastGrd<1
				&&pVehicle->AxleSet.Axle[1].AxleType==2
				&&pVehicle->AxleSet.Axle[2].AxleType==1)
			{
				//20171116新增客4判断
				pVehicle->JudgeSet.u8JudgePattern=FOURTH_KECHE;
				pVehicle->JudgeSet.u8JudgeRule=203;
			}
			else
			{
...
void VehPatternModify(VehicleSet *pVehicle)
{
		...
		&&(pVehicle->Dp.DpHeight1_2>0||pVehicle->Dp.DpHeight2_3>0))
	{
		...
		if ((pVehicle->FeatureSet.IsCargo>0&&pVehicle->FeatureSet.IsSideUiform<1)
			||(pVehicle->FeatureSet.IsCastGrd>0&&pVehicle->EdgeSet.u16VehHeight>1900))
		{
			if (pVehicle->JudgeSet.u8JudgePattern<FIFTH_HUOCHE)
			{
				if (pVehicle->EdgeSet.u16HeadHeight <= 2200)  //车高2200以下	
		...
		if (pVehicle->FeatureSet.IsContainer>0&&pVehicle->EdgeSet.u16VehHeight>1900)
		{
			//货箱
			if (pVehicle->JudgeSet.u8JudgePattern < FIFTH_HUOCHE)
			{
		...
		if (pVehicle->FeatureSet.IsFillWin>0&&(pVehicle->FeatureSet.IsHeadJump
			||(pVehicle->FeatureSet.IsSideUiform&&pVehicle->EdgeSet.u16VehHeight>1800)))
		{
			//封闭货车 //20171115 封闭车窗+侧面一致+车高约束判货车
			if (pVehicle->JudgeSet.u8JudgePattern < FIFTH_HUOCHE)
			{
				if (pVehicle->EdgeSet.u16HeadHeight <= 2200)  //车高2200以下	
				{
