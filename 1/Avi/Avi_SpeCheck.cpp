...
void PickUpFramCal(VehicleSet *pVehicle)
{
	...
	int32   l_n32RightZData    = 0;  //右侧挡板最高点
	int32   l_n32RightXData    = 0;  //右侧挡板最低点
	int32   l_n32RightXLow     = 0;  //右侧挡板最低点
	int32   l_n32RightZLow     = 9999;  //右侧挡板最低点
	...
	for (i16TmpIndex1=1;i16TmpIndex1<i16BreakI;i16TmpIndex1++)
	{
		...
		if (pVehicle->CurRegion.xdata[i16TmpIndex1]<l_n32LeftXData+150)
		{
			//20171119 增加客车车窗完全打穿类似车斗情况的剔除 
			return;
		}
		...
		if (pVehicle->CurRegion.xdata[i16TmpIndex1]+150>pVehicle->CurRegion.xMax)
		{
			...
			if (l_n32RightZLow>pVehicle->CurRegion.zdata[i16TmpIndex1])
			{
				//20171115 针对无斗中部点情况
				l_n32RightZLow=pVehicle->CurRegion.zdata[i16TmpIndex1];
				l_n32RightXLow=pVehicle->CurRegion.xdata[i16TmpIndex1];
	...
	if (i16DouCnt > 0) 
	{
		...
		if ((l_n32RightXData - l_n32LeftXData > 1000) && i16BreakX < l_n32RightXData
			&&  (l_n32RightZData-i16DouHt > 250) && i16DouHt > 500 
			&& (l_n32LeftZData-i16DouHt > 250) && i16DouHt <1500&& i16ErrPt<5 && i16DafeiCnt<5) //
		{
	...
	{
		//激光安装较矮 无斗中部点 20171115
		if ((l_n32RightXData - l_n32LeftXData > 1000) && i16BreakX +100 >l_n32RightXData
			&&  (l_n32RightZData-l_n32RightZLow > 150) && l_n32RightZLow > 500 
			&& (l_n32LeftZData-l_n32RightZLow > 150) && l_n32RightZLow <1500 
			&& i16LeftI-i16BreakI<3 && ABS(l_n32LeftZData-l_n32RightZData)<100) //
		{
			pVehicle->SpeVSet.u8PickUpFlag[CurFramI]=1;
		}
	}
...
void PickUpCheck(VehicleSet *pVehicle)
{
	...
	for (u16Index=0;u16Index<pVehicle->u16FrameCnt;u16Index++)
	{
		if (StaI<1)
		{
			...
			//else if (u16Index>pVehicle->u16FrameCnt*2/3)
			//{
			//	//20170904修改超过1/2为超过2/3
			//	StaI=u16Index;
			//}
		}
		else
		{
			...
			if (pVehicle->EdgeSet.u16MaxHeight[u16Index]-pVehicle->FrShape.u16LeftMaxHt[u16Index]>500
				&&pVehicle->EdgeSet.u16MaxHeight[u16Index]>1000&&pVehicle->FrShape.u16LeftMaxHt[u16Index]>1000)
			{
				//20171116 增加左线与车高线高度差距大 判为载货
				PickFrCnt++;
			}
		}
	}
...
void EnCloseWinCal(VehicleSet *pVehicle)
{
	...
	//找到终止段索引
	EndI=0;
	for (i16Index=2;i16Index<PtSum;i16Index++)
	{
		if (pVehicle->CurRegion.zdata[i16Index]< pVehicle->EdgeSet.u16MaxHeight[CurFramI]-100
			&&pVehicle->CurRegion.zdata[i16Index]+500> pVehicle->EdgeSet.u16MaxHeight[CurFramI]
			&&pVehicle->CurRegion.xdata[i16Index]<CastX+300)
		{
			if (ABS(pVehicle->CurRegion.zdata[i16Index]-pVehicle->CurRegion.zdata[i16Index-1])<51
				&&ABS(pVehicle->CurRegion.xdata[i16Index]-pVehicle->CurRegion.xdata[i16Index-1])<51)
			{
				FitCnt++;
			}
			else
...
void EnCloseWinCheck(VehicleSet *pVehicle)
{
	...
	uint8 CheckType=0; //1--只检测出第一轴或无轴；2--只检测出最后一轴或两轴均检测到
	...
	{
		StaI=pVehicle->AxleSet.Axle[pVehicle->AxleSet.AxleSum-1].AxleOriStaI;//原始轮轴起点
		TmpI=StaI;
		for (i16Index=pVehicle->AxleSet.Axle[pVehicle->AxleSet.AxleSum-1].AxleOriStaI;i16Index>1;i16Index--)
		{
			if ((pVehicle->SideSet.SideFlag[i16Index]&2)<1)
			{
				TmpI=i16Index;
				break;
			}
		...
		if (StaI-TmpI<pVehicle->AxleSet.Axle[pVehicle->AxleSet.AxleSum-1].AxleOriEndI-pVehicle->AxleSet.Axle[pVehicle->AxleSet.AxleSum-1].AxleOriStaI+1)
		{
			return;
		}
		else
	...
	//计算货箱的高度 采用统计规律实现 20171116 采用统计规律判断后部的平整性
	...
	for (i16Index=StaI;i16Index<EndI;i16Index++)
	{
		if (pVehicle->EdgeSet.u16MaxHeight[i16Index]+150>ContainerHt)
		{
			CheckCnt++;
	...
	if (CheckCnt && FitCnt*100/CheckCnt>90 && DisFitCnt*100/CheckCnt<30)
	{
