...
int pre_process()
{
	...
	if(point_count>2)
	{
		...
		if ((beyond_DOUBLE_AXLE >3 && point_below_height[1] - min_dis_X_t[1] > 110)
			|| (beyond_DOUBLE_AXLE > 1 && point_below_height[1] - min_dis_X_t[1] > 210)
			|| (beyond_DOUBLE_AXLE > 6 && point_below_height[1] - min_dis_X_t[1] > 180)
			)
			belowHeigh_X_t[3] = belowHeigh_X_t[1] - belowHeigh_X_t[2];
		else
	...
	if( neighout_max_X - neighout_min_X > 45)
	{
		...
		if (height_over_count <= 1 && axle_count == 0  && ( (Laser_Height > 1700)  || ( Laser_Height > 1100 && Mid_Point_Cal > 280 ) )  )
		{
			for(index = 2; index <= index_extract; index++)
			{
				...
				if ((ABS(min_dis_X[8][index]) > 2200 || (ABS(min_dis_X[8][index]) < 300 && ABS(min_dis_X[8][index]) > 0)) && index <= index_top)
			...
			|| ( index_extract-2 >=index_top &&  (min_dis_X[3][index_extract-1]  - axle_min_X) > MAX_Height/2 &&  (min_dis_X[3][index_extract-1] - min_dis_X[3][index_extract-2])  > MAX_Height/3 && (min_dis_X[3][index_extract] - min_dis_X[3][index_extract-1])  < (min_dis_X[3][index_extract-1] - min_dis_X[3][index_extract-2])   ) )
		{
			...
			if (index_top < index_extract  && index_extract > 3 && index_top > 1 )  //出轴判定
			{
					...
					//	%%寻找到轮轴
				{
					...
					for(index =index_axle_start; index <=index_axle_end; index++ )
					{
						if (min_dis_X[1][index] < (axle_min_X+MAX_Height/4) && belowHeigh_X[index][2]>0 )  //%计算极值点处 600mm以下到激光器的最近距离,轮胎最大宽度
						{
					...
					if (((belowHeigh_X_count * 10 >= (index_axle_end - index_axle_start + 1)) || car_axle) && belowHeigh_X_count > 0)
					{
					...
					threshold_tyre = 65 * belowHeigh_X_avg / (Laser_Height - 65) + 70;

					if ((car_height != 1 && car_axle != 1 && (axle_min_X  <  belowHeigh_X_avg + threshold_tyre && axle_min_X  >  belowHeigh_X_avg - 50)) || ((car_axle || car_height) && (axle_min_X  <  belowHeigh_X_avg + 120 && axle_min_X  >  belowHeigh_X_avg - 50))
						|| (temp_count > 6 && temp_count2 > 6 && axle_min_X  <  belowHeigh_X_avg + threshold_tyre + 10 && axle_min_X  >  belowHeigh_X_avg - 50))
					{
						if(axle_count)
						{
							...
							if (belowHeigh_width_max > DOUBLE_AXLE &&  car_height == 0 && min_dis_X[9][index_axle_end] - min_dis_X[9][index_axle_start] <3)
								fake_axle = 2;
							if ((car_height || car_axle) && axle_min_X < 2000 && axle_min_X - axle_index[1][4] >90 + (index_axle_end - axle_index[axle_count][2]) * 4 / 5)//小车的两个车轮之间的axle_min_X关系
								fake_axle = 3;
							if ((car_height || car_axle) && (index_top - axle_index[axle_count][2]) < (axle_index[axle_count][5] + axle_index[axle_count][6]) * 16 / 10) //小车两轴间隔距离很近 多轴
								fake_axle = 6;
							if (car_height == 0 && axle_min_X > axle_index[axle_count][4] + 200 && (min_dis_X[9][index_top] - axle_index[axle_count][9])<last_axle_frame_count * 6 / 5 && (last_axle_frame_count <= (axle_index[axle_count][6] + axle_index[axle_count][5]) * 35 / 10))
								fake_axle = 4;
							...
							////	fake_axle = 6;
							if ( /*车速较慢*/ (last_axle_frame_count > (axle_index[axle_count][6] + axle_index[axle_count][5]) * 23 / 10) && (last_axle_frame_count  <  (axle_index[axle_count][6] + axle_index[axle_count][5]) * 42 / 10)
								&&/*半个车轮范围内*/  (last_axle_frame_count / 4 + axle_index[axle_count][9])>min_dis_X[9][index_top]
								&& (!(car_height || car_axle))
								)
								fake_axle = 10;
							
							if (axle_count == 3 && axle_index[axle_count - 1][3] == 2 && axle_index[axle_count ][3] == 2 && belowHeigh_width_max <  DOUBLE_AXLE								
								&&min_dis_X[9][index_axle_end] - axle_index[axle_count][9]  < (min_dis_X[9][index_axle_end] - min_dis_X[9][index_axle_start]  ) *5/2 )//连续的两个双胎后不会出现单胎
							{
								fake_axle = 11;
							}
						...
						if ((belowHeigh_X_max - belowHeigh_X_min > 210 && temp_count < 6 && temp_count2 < 6) || belowHeigh_X_max - belowHeigh_X_min > 320) // 车轮的平整度
							fake_axle = 9;
						...
						if(fake_axle == 0)
						{
							...
							if (belowHeigh_width_max< DOUBLE_AXLE)
...
/* 定位地面投影点                                                       */
/************************************************************************/
void GetGroundPoint(VehicleSet *pVehicle)
{
	pVehicle->EdgeSet.u16GrdxLine[pVehicle->u16FrameCnt-1]=min_dis_X_t[1];
}
