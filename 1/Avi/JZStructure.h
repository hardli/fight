...
typedef struct tagSystemParam	
{
	...
	uint8   u8WorkMode;			 //工作模式 0--现场运行；1--模拟发送运行;2--加载波形;
...
typedef struct tagFramInfo
{
	...
	uint8   u8FitLow;//满足最低点要求;
	uint8   u8FitMid;//满足中等点要求;
...
typedef struct tagArcSet
{
	...
	uint16 ArcThreshH;//车辆高度
	uint8  startheightflag;
	uint16 ArcXLine[FRAME_MAXCNT];//存储车辆一定高度数据
...
{
	uint8 u8State;//过车状态
	uint8 u8VoutFlag;//车辆出车类型 0--正常出车；1--强制出车；2--强制失败
	
	uint8 u8FitLow;//满足低等点要求
	uint8 u8FitMid;//满足中等点要求
	uint16 u16LeftX;//最左X值
	uint16 u16RightX;//最右X值
