...
void ContainerCheck(VehicleSet *pVehicle)
{
	...
	if (pVehicle->AxleSet.AxleSum>1&&pVehicle->AxleSet.Axle[0].AxleOriEndI<pVehicle->AxleSet.Axle[pVehicle->AxleSet.AxleSum-1].AxleOriStaI
		&& pVehicle->u16FrameCnt/2-pVehicle->AxleSet.Axle[pVehicle->AxleSet.AxleSum-1].AxleOriStaI>0)
	{
		//20171119 修改第二轴停车情况的判断
...
void ContainerCal(VehicleSet *pVehicle)
{
	...
	{
		//20171116 修改进入检测条件为轴数大于0或车高大于1700
		return;
	}
...
void VehHeightModify(VehicleSet *pVehicle)
{
	...
	for (l_u16Index = 1; l_u16Index < pVehicle->u16FrameCnt; l_u16Index++)
	{
		...
		{
			if (pVehicle->EdgeSet.u16MaxHeight[l_u16Index]>pVehicle->EdgeSet.u16MaxHeight[l_u16Index-1]+950
...
void AxleHopperCheckEx(VehicleSet *pVehicle,uint16 *pH)
{
	...
	for (iIndex=0;iIndex<PtNum;iIndex++)
	{
		...
		if (EndIndex == 0 && pH[pVehicle->u16FrameCnt-iIndex]>HopperTh
			&& pVehicle->EdgeSet.u16MinXEdge[pVehicle->u16FrameCnt-iIndex]-300<pVehicle->AxleSet.Axle[1].AxleCastX)
		{
	...
	for (iIndex=StaIndex;iIndex<EndIndex+1;iIndex++)
	{
		if (pH[iIndex]>0)
		{
			...
			if (pVehicle->EdgeSet.u16MaxHeight[iIndex]+1>pVehicle->EdgeSet.u16VehHeight)
			{
				//20171116 增加要求 最大高不能出现在车斗判定段
				return;
			}
		}
	}
	...
	if (pVehicle->AxleSet.Axle[pVehicle->AxleSet.AxleSum-1].AxleCastX+150>HopperCI.AvgX)
	{
			...
			&& HopperCI.MaxZ-HopperCI.AvgH>400 && HopperCI.AvgH-HopperCI.MinZ<100)
		{
			...
			&& HopperCI.MaxZ-HopperCI.AvgH>800 && HopperCI.AvgH-HopperCI.MinZ>500)
		{
			//最大高出现在第二轴上，且呈现下降趋势，整体高度落差有800 20171114
			pVehicle->FeatureSet.IsHopper=pVehicle->FeatureSet.IsHopper|4;
			return;
		}
...
void GlassWinCheck(VehicleSet *pVehicle)
{
	...
	for (l_u16Index = StaI; l_u16Index < EndI; l_u16Index++) //检测后一半帧数	
	{
		...
		{
			if (pVehicle->EdgeSet.u16HeadHeight<1900)
			{
				//车头高由1800->1900 20171116 区分大小客车
...
void HeadJumpCheckEx(VehicleSet *pVehicle,uint16 *pH)
{
	...
	for (u16Index=u16StaI;u16Index<u16EndI;u16Index++)
	{
		...
		//20171116 加入轮轴对应段为倾斜直线的情况的判断剔除
		if (pH[u16Index]<AxMinZ)
		{
			AxMinZ=pH[u16Index];
		}

		if (pH[u16Index]>AxMaxZ)
		{
			AxMaxZ=pH[u16Index];
		}

		if (pH[u16Index+1]+20<pH[u16Index])
		{
			u16DownCnt++;
		}
	}
	...
	//20171116 加入轮轴对应段为倾斜直线的情况的判断剔除
	if (AxMaxZ-AxMinZ>380&&100*u16DownCnt/(u16EndI-u16StaI)<5)
	{
		return ;
	}
...
void FindHeadJump(VehicleSet *pVehicle,uint8 nMode)
{
	...
	if (pVehicle  ==  NULL||pVehicle->u16FrameCnt > FRAME_MAXCNT || pVehicle->u16FrameCnt < 10
		...
		||pVehicle->EdgeSet.u16VehHeight<1800 || pVehicle->AxleSet.AxleSum>2)
	...
	//20171116加入停车检测 动态调整判断范围
	if (pVehicle->AxleSet.Axle[1].AxleOriStaI<pVehicle->u16FrameCnt/2)
	{
		i16CheckEd=pVehicle->AxleSet.Axle[1].AxleOriStaI;
	}
	else if(pVehicle->AxleSet.Axle[0].AxleOriEndI>pVehicle->u16FrameCnt/2)
	{
		return;
	}
	else
	{
		i16CheckEd=pVehicle->u16FrameCnt-1;
	}

	
	//找到车高平坦段
	for (i16Index=i16CheckEd;i16Index>i16CheckEd/3;i16Index--)
	{
...
void FramShapeCal(VehicleSet *pVehicle)
{
	...
	for (i16Index=i16PtSum;i16Index>1;i16Index--)
	{
		...
		//20171119 增加判断500高度以上的最左x作为边界
		if (pVehicle->CurRegion.zdata[i16Index]>500)
		{
			if (pVehicle->CurRegion.xdata[i16Index]<i16MinX)
			{
				i16MinX=pVehicle->CurRegion.xdata[i16Index];
			}
		}
	}
	//记录打飞情况
	...
	//if (pVehicle->FrShape.BaseX<1)
	//{
	//	pVehicle->FrShape.BaseX=pVehicle->CurRegion.xMin;
	//}
	//else
	//{
	//	//20170904 扩大边缘延续容忍度为300防止最小点孤立引起误判
	//	if (ABS(pVehicle->CurRegion.xMin-pVehicle->FrShape.BaseX)<300)
	//	{
	//		pVehicle->FrShape.BaseX=pVehicle->CurRegion.xMin;
	//	}
	//}
	
	//直接取最左侧300cm作为左侧切片 20170904
	if (i16MinX<9999)
	{
		pVehicle->FrShape.BaseX=i16MinX;
	}
	else
		pVehicle->FrShape.BaseX=pVehicle->CurRegion.xMin;
	...
	for (i16Index=i16PtSum;i16Index>0;i16Index--)
	{
		...
		//20171116 修改切片距离300->350
		if (pVehicle->CurRegion.xdata[i16Index]<pVehicle->FrShape.BaseX+350)
		{
...
void CastGrdCheckCal(VehicleSet *pVehicle)
{
	...
	{
		if (pVehicle->CurRegion.zdata[i16Index]>VEHTH_LOW
			&&pVehicle->CurRegion.zdata[i16Index]<800)
		{
			FitPtCnt++;
		}
...
void CargoXEdgeCheck(VehicleSet *pVehicle)
{
	...
	if ((u16TailCastX-300>u16CheckAvgX && u16TailCastX-500>u16CheckMinX)&&u16CheckAvgX)
	{
...
void CastGroundCheck(VehicleSet *pVehicle)
{
	...
	uint16  MinGrdX=9999;//栏杆段 接地最小X
	uint16  MaxGrdX=0;//栏杆段 接地最大X
	uint16  StaI=0;
	uint16  EndI=0;
	...
	//一般出现中间穿透至地上的时，其前部出现较大空隙，两线差值较大
	for (l_u16Index=StaI;l_u16Index<EndI;l_u16Index++)
	{
		if (SureFlag<1 && pVehicle->EdgeSet.u16MinXEdge[l_u16Index]+300<pVehicle->EdgeSet.u16GrdxLine[l_u16Index])
		{
			//找到前方大片空隙
			SureFlag=1;
		}
		else if (SureFlag&&pVehicle->EdgeSet.u8CastGrdFlag[l_u16Index]>2
			&& pVehicle->EdgeSet.u16MaxHeight[l_u16Index]>1000)
		{
			//找到中间穿透到地上帧
			CastFrCnt++;

			//20171115 通过接地线的平直来判断底部横杆
			if (MinGrdX>pVehicle->EdgeSet.u16GrdxLine[l_u16Index])
			{
				MinGrdX=pVehicle->EdgeSet.u16GrdxLine[l_u16Index];
			}

			if (MaxGrdX<pVehicle->EdgeSet.u16GrdxLine[l_u16Index])
			{
				MaxGrdX=pVehicle->EdgeSet.u16GrdxLine[l_u16Index];
			}
		}
	}
	
	if (CastFrCnt>1 && MaxGrdX-MinGrdX<150)
	{
		//栏杆很平
		pVehicle->FeatureSet.IsCastGrd=1;
	}
	else if (CastFrCnt>1 && MaxGrdX-MinGrdX>300)
	{
		//非栏杆 但接地线参差不齐
		pVehicle->FeatureSet.IsCastGrd=2;
	}
}
...
void MutiFeatureFusion(VehicleSet *pVehicle)
{
	...
	AxleHopperCheckEx(pVehicle,pVehicle->EdgeSet.u16MaxHeight);
...
void DirJudge_Coil(VehicleSet *pVehicle)
{
	...
	if(pVehicle->DirSet.u8CL_StateHi<1&&pVehicle->DirSet.u8CL_StateLv<1&&pVehicle->DirSet.u8CL_StateLw<1)
	{
		pVehicle->DirSet.u8VehDir=MOVE_NONO;//出现3个均为计数 给无方向
	}
	else if((pVehicle->DirSet.u8CL_StateHi<1) && (pVehicle->DirSet.u8CL_StateLv <= g_VehOutSet.u8DirLvCnt))  //倒车后退被激光扫描到又前进到发卡机
	{
...
/* 直接通过变化来判断方向                                               */
/************************************************************************/
void DirCheckDirect(VehicleSet *pVehicle)
{
	//	int16 AxleStaX=0;
	//	int16 AxleEndX=0;
	//	int16 Axle0MaxX=0;
	...
	//	
	//	//第一胎为双胎倒车
	//	if (pVehicle->AxleSet.Axle[0].AxleType==2
	//		&&pVehicle->AxleSet.Axle[pVehicle->AxleSet.AxleSum-1].AxleType==1)
	//	{
	//		pVehicle->DirSet.u8LogDir=MOVE_BACK;
	//		return;
	//	}
	
	//	//计算第一轴前的车地分界线平均位置
	//	A0StaI=1;
	//	A0EndI=pVehicle->AxleSet.Axle[0].AxleOriStaI-1;
	//	A0PtSum=A0EndI-A0StaI+1;
	//	A0BaseX=pVehicle->AxleSet.Axle[0].AxleCastX;
	//	FitCnt=0;
	//	for (TmpI=A0StaI;TmpI<A0EndI;TmpI++)
	//	{
	//		if (pVehicle->EdgeSet.u16GrdxLine[TmpI]-50>A0BaseX &&pVehicle->EdgeSet.u16GrdxLine[TmpI]-1500< A0BaseX)
	//		{
	//			if (Axle0MaxX<pVehicle->EdgeSet.u16GrdxLine[TmpI])
	//			{
	//				Axle0MaxX=pVehicle->EdgeSet.u16GrdxLine[TmpI];
	//			}
	//		}
	//	}
	
	
	//	//for (TmpI=A0StaI;TmpI<pVehicle->AxleSet.Axle[0].AxleOriStaI/2;TmpI++)
	//	//{
	//	//	//后视镜帧判断 
	//	//	//if (((pVehicle->EdgeSet.u16MinXEdge[TmpI]<A0BaseX && pVehicle->EdgeSet.u16MinXEdge[TmpI]+100>A0BaseX)
	//	//	//	||(pVehicle->EdgeSet.u16MinXEdge[TmpI]>A0BaseX && pVehicle->EdgeSet.u16MinXEdge[TmpI]-100<A0BaseX))
	//	//	//	&& pVehicle->EdgeSet.u16MinXEdge[TmpI]+1500<pVehicle->EdgeSet.u16GrdxLine[TmpI]
	//	//	//    && pVehicle->EdgeSet.u16MaxHeight[TmpI]+500>pVehicle->EdgeSet.u16HeadHeight)
	//	//	//{
	//	//	//	A0MirFlag++;
	//	//	//}
	...
	//	//	if (pVehicle->EdgeSet.u16GrdxLine[TmpI]-50>A0BaseX &&pVehicle->EdgeSet.u16GrdxLine[TmpI]-1500< A0BaseX)
	//	//	{
	//	//		DeltX=pVehicle->EdgeSet.u16GrdxLine[TmpI]-pVehicle->EdgeSet.u16GrdxLine[TmpI-1];
	//	//		if (ABS(DeltX)<200)
	//	//		{
	//	//			FitCnt++;
	//	//		}
	...
	//	//FitCnt=0;
	//	//for (TmpI=A0EndI;TmpI>pVehicle->AxleSet.Axle[0].AxleOriStaI/2;TmpI--)
	//	//{
	//	//	if (pVehicle->EdgeSet.u16GrdxLine[TmpI]-50>A0BaseX &&pVehicle->EdgeSet.u16GrdxLine[TmpI]-1500< A0BaseX)
	//	//	{
	//	//		DeltX=pVehicle->EdgeSet.u16GrdxLine[TmpI]-pVehicle->EdgeSet.u16GrdxLine[TmpI-1];
...
//	{
//		DeltX=pVehicle->EdgeSet.u16GrdxLine[TmpI]-pVehicle->EdgeSet.u16GrdxLine[TmpI-1];
//		if (ABS(DeltX)>A0DiffMax)
//		{
//			A0DiffMax=ABS(DeltX);
//		}
//		AvgH0+=pVehicle->EdgeSet.u16MaxHeight[TmpI];
//	}

//	AxleStaX=AverageU(&pVehicle->EdgeSet.u16GrdxLine[A0StaI],A0EndI-A0StaI+1);
//	if (A0EndI-A0StaI>0)
//	{
//		AvgH0=AvgH0/(A0EndI-A0StaI);
//	}
//	
//	//计算最后一轴的车地分界线

//	AnStaI=pVehicle->AxleSet.Axle[pVehicle->AxleSet.AxleSum-1].AxleOriEndI+1;
//	AnEndI=pVehicle->u16FrameCnt-1;
//	AnPtSum=AnEndI-AnStaI+1;
//	AnBaseX=pVehicle->AxleSet.Axle[pVehicle->AxleSet.AxleSum-1].AxleCastX;
//	FitCnt=0;
//	for (TmpI=AnStaI;TmpI<pVehicle->AxleSet.Axle[pVehicle->AxleSet.AxleSum-1].AxleOriEndI+AnPtSum/2;TmpI++)
//	{
//		if (pVehicle->EdgeSet.u16GrdxLine[TmpI]-50>AnBaseX &&pVehicle->EdgeSet.u16GrdxLine[TmpI]-1500< AnBaseX)
//		{
//			DeltX=pVehicle->EdgeSet.u16GrdxLine[TmpI]-pVehicle->EdgeSet.u16GrdxLine[TmpI-1];
//			if (ABS(DeltX)<200)
//			{
...
//	FitCnt=0;
//	for (TmpI=AnEndI;TmpI>pVehicle->AxleSet.Axle[pVehicle->AxleSet.AxleSum-1].AxleOriEndI+AnPtSum/2;TmpI--)
//	{
//		//后视镜帧判断
//		//if (((pVehicle->EdgeSet.u16MinXEdge[TmpI]<AnBaseX && pVehicle->EdgeSet.u16MinXEdge[TmpI]+100>AnBaseX)
//		//	||(pVehicle->EdgeSet.u16MinXEdge[TmpI]>AnBaseX && pVehicle->EdgeSet.u16MinXEdge[TmpI]-100<AnBaseX))
//		//	&& pVehicle->EdgeSet.u16MinXEdge[TmpI]+1500<pVehicle->EdgeSet.u16GrdxLine[TmpI]
//		//    && pVehicle->EdgeSet.u16MaxHeight[TmpI]+500>pVehicle->EdgeSet.u16HeadHeight)
//		//{
//		//	AnMirFlag++;
...
//	//从起止段中寻找相邻差分最大值
//	for (TmpI=AnStaI;TmpI<AnEndI;TmpI++)
//	{
//		DeltX=pVehicle->EdgeSet.u16GrdxLine[TmpI]-pVehicle->EdgeSet.u16GrdxLine[TmpI-1];
//		if (ABS(DeltX)>AnDiffMax)
//		{
//			AnDiffMax=ABS(DeltX);
//		}
//		AvgHE+=pVehicle->EdgeSet.u16MaxHeight[TmpI];
//	}

//	AxleEndX=AverageU(&pVehicle->EdgeSet.u16GrdxLine[AnStaI],AnEndI-AnStaI+1);
//	if (AnEndI-AnStaI>0)
//	{
//		AvgHE=AvgHE/(AnEndI-AnStaI);
//	}


//	if (pVehicle->EdgeSet.u16HeadHeight<1600)
//	{
//		//头尾底盘高接近时，看相邻帧X差值  前后高度差优先 其次是相邻deltX  最后是平均接地X
//		if (AvgHE>AvgH0&&AvgH0+300<AvgHE)
//		{
//			//前部均告比后部低300高度以上
//			pVehicle->DirSet.u8LogDir=MOVE_FRNT;
//		}
//		else if (AvgH0>AvgHE&&AvgHE+300<AvgH0)
//		{
//			//前部均告比后部均高大300
//			pVehicle->DirSet.u8LogDir=MOVE_BACK;
//		}
//		else if (AxleStaX<AxleEndX&&AxleEndX-AxleStaX>150)
//		{
//			//第一轴前底盘低于最后一轴底盘高度 为前进
//			pVehicle->DirSet.u8LogDir=MOVE_FRNT;
//		}
//		else if (AxleEndX<AxleStaX&&AxleStaX-AxleEndX>150)
//		{
//			//第一轴前底盘高于最后一轴底盘高度 为倒车
//			pVehicle->DirSet.u8LogDir=MOVE_BACK;
//		}
//		else
//		{
//			pVehicle->DirSet.u8LogDir=MOVE_NONO;
//		}
//	}
//	else 
//	{
//		////头尾底盘高接近时，看相邻帧X差值
//		if (AxleStaX<AxleEndX&&AxleEndX-AxleStaX>300)
//		{
//			//第一轴前底盘低于最后一轴底盘高度 为前进
//			pVehicle->DirSet.u8LogDir=MOVE_FRNT;
//		}
//		else if (AxleEndX<AxleStaX&&AxleStaX-AxleEndX>300)
//		{
//			//第一轴前底盘高于最后一轴底盘高度 为倒车
//			pVehicle->DirSet.u8LogDir=MOVE_BACK;
//		}
